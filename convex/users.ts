...
export const getUserById = query({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            // Return user without modified imageUrl in case of error
            return user;
        }
    },
});
...
export const searchUsers = query({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            // Add logging to debug the search results
            console.log(`Search query: "${args.search}" returned ${users.length} results`);
            ...
            const usersWithImage = await Promise.all(
                users.map(async (user) => {
                    ...
                    try {
                        ...
                        console.error('Error getting storage URL for user image:', error);
                        return user;
                    }
                })
            );
